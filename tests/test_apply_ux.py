...
def test_register_wizard_has_three_steps() -> None:
    source = _read("app.py")
    start = source.index("REGISTER_WIZARD_STEPS = (")
    end = source.index(")", start)
    body = source[start : end + 1]
    assert body.count("auth.register.wizard.") == 3
    ...
    assert "auth.register.wizard.language" not in body
    assert "auth.register.wizard.preferences" not in body
