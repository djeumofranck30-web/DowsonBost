"""Analysis depth volumes: rapide 25, standard 60, complet 100, etendu 150."""
...
from constants import ANALYSIS_DEPTH_OPTIONS, ANALYSIS_DEPTH_POOL, ANALYSIS_DEPTH_TOP
...
def test_depth_volumes_are_25_60_100_150() -> None:
    assert ANALYSIS_DEPTH_OPTIONS == ("rapide", "standard", "complet", "etendu")
    assert ANALYSIS_DEPTH_POOL == {"rapide": 25, "standard": 60, "complet": 100, "etendu": 150}
    assert ANALYSIS_DEPTH_TOP == {"rapide": 25, "standard": 60, "complet": 100, "etendu": 150}
...
def test_depth_labels_show_offer_counts() -> None:
    ...
    assert "150" in analysis_depth_label("etendu")
...
def test_ui_starts_analysis_immediately() -> None:
    ...
    assert "idle_sleep: float = 2.0" in worker
...
def test_display_limit_follows_selected_depth() -> None:
    ...
    assert matching_display_limit({"analysis_depth": "etendu"}) == 150
    ...
    assert len(cap_results_to_requested_best(found_online, {"analysis_depth": "etendu"})) == 150
...
def test_enqueue_etendu_stores_150_offers(sqlite_db) -> None:
    from auth import authenticate_user, register_user

    init_persistence_tables()
    ok, msg = register_user(
        "Jane Doe",
        "jane.etendu@example.com",
        "Secret123!",
        target_job_title="Developer",
        contract_type="CDI",
        experience_level="confirme",
    ...
    assert ok, msg
    ok_login, _, user = authenticate_user("jane.etendu@example.com", "Secret123!")
    assert ok_login and user is not None
    job_id, err = enqueue_analysis_job(
        int(user["id"]),
        {
            "id": int(user["id"]),
            "full_name": "Jane Doe",
            "email": "jane.etendu@example.com",
            "target_job_title": "Developer",
            "contract_type": "CDI",
            "country": "France",
        },
        job_provider="adzuna",
        analysis_depth="etendu",
        cv_fingerprint="fp-etendu",
        cv_text="CV texte de Jane " * 10,
    )
    assert err == ""
    stored = get_analysis_job(job_id, int(user["id"]))
    assert stored is not None
    assert stored["analysis_depth"] == "etendu"
    assert int(stored["matching_pool"]) == 150
    assert int(stored["matching_top"]) == 150
