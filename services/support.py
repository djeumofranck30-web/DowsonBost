"""1:1 candidate ↔ admin support chat."""
...
def send_user_support_message(user_id: int, body: str) -> tuple[bool, str, dict[str, Any] | None]:
    ...
    message = insert_support_message(int(user_id), SENDER_USER, text)
...
def user_support_thread(user_id: int) -> list[dict[str, Any]]:
    return list_support_thread(int(user_id))


def admin_support_thread(user_id: int) -> list[dict[str, Any]]:
    if not get_user_by_id(int(user_id)):
        return []
    return list_support_thread(int(user_id))


def mark_user_support_read(user_id: int) -> None:
    mark_support_thread_read(int(user_id), incoming_role=SENDER_ADMIN)


def mark_admin_support_read(user_id: int) -> None:
    mark_support_thread_read(int(user_id), incoming_role=SENDER_USER)
